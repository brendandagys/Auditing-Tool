...
class EntryTopLevel(tk.Toplevel):
    ...
    def handle_listbox_creation(self):
        ...
        self.frame_scrollbar_horizontal = tk.Scrollbar(
            self, orient='horizontal')
        self.frame_scrollbar_horizontal.pack(side='bottom')

        self.top_lbox = tk.Listbox(self, font=('courier', 8), width=168, height=50, activestyle='none',
                                   yscrollcommand=self.frame_scrollbar.set, xscrollcommand=self.frame_scrollbar_horizontal.set)
        self.top_lbox.pack()
        ...
        header_string = '        ' + 'DATE'.ljust(15) + 'ID'.ljust(16) + 'TRAY NAME'.ljust(
            tray_name_longest_length+5) + 'PREPARED BY'.ljust(30) + 'Q1'.ljust(8) + 'Q2'.ljust(8)
        header_string += 'Q3'.ljust(8) + 'Q4'.ljust(8) + \
            'Q5'.ljust(8) + 'Q6'.ljust(8) + 'COMMENT'.ljust(100)

        header_gap_string = ''.ljust(17) + ''.ljust(16) + ''.ljust(
            tray_name_longest_length+5) + ''.ljust(30) + ''.ljust(8) + ''.ljust(8)
        header_gap_string += ''.ljust(8) + ''.ljust(8) + \
            ''.ljust(8) + ''.ljust(8) + ''.ljust(100)
        ...
        for row in range(self.frame.df.shape[0]):
            row_string = ('(' + str(index_number) + ')').rjust(6)
            index_number += 1
            row_string += '  ' + \
                str(self.frame.df.iloc[row, 0]).ljust(15)  # date
            row_string += (str(self.frame.df.iloc[row, 1].split(
                '[')[1]).replace(']', ''))[:12].ljust(16)  # ID
            # tray name
            row_string += self.frame.df.iloc[row,
                                             1].ljust(tray_name_longest_length+5)
            row_string += self.frame.df.iloc[row,
                                             2][:25].ljust(30)  # prepared by
            # external wrap intact
            row_string += self.frame.df.iloc[row, 3].ljust(8)
            # tray identified
            row_string += self.frame.df.iloc[row, 4].ljust(8)
            # proper quantity sizing
            row_string += self.frame.df.iloc[row, 5].ljust(8)
            # chemical indicator included
            row_string += self.frame.df.iloc[row, 6].ljust(8)
            # instuments visually clean
            row_string += self.frame.df.iloc[row, 7].ljust(8)
            # instruments processed correctly
            row_string += self.frame.df.iloc[row, 8].ljust(8)
...
class SampleApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.employee_list = ['Employee (1)', 'Employee (2)', 'Employee (3)', 'Employee (4)', 'Employee (5)',
        ...
        # Window settings
        self.winfo_toplevel().title('Audit Tool')
    ...
    def initialize_tray_list(self):
        try:
            self.tray_df = pd.read_excel(
                'C:\\Path\\To\\Excel_file.xlsx')
            self.tray_list = list(self.tray_df['Tray Name'])
        except Exception:
            self.tray_list = ['Item 1', 'Item 2', 'Item 3', 'Item 4',
    ...
    def prepare_dataframe(self, employee_name):
        ...
        try:
            self.df = pd.read_excel(
                'C:\\Path\\To\\audit_data.xlsx' + CURRENT_YEAR + ' - ' + employee_name + '.xlsx')

        except:
            # tk.messagebox.showinfo('Data Load Error', 'Cannot find master data file. Please contact Brendan.')
            self.df = pd.DataFrame(columns=['Date', 'Tray Name', 'Prepared By', 'External Wrap Intact', 'Tray Identified', 'Proper Quantity/Sizing',
                                   'Chemical Indicator Included', 'Instruments Visually Clean', 'Instruments Processed Correctly', 'Comment'])

    def handle_menu_bar_creation(self):

        self.menubar = tk.Menu(self)
        self.menubar.add_command(
            label='                            CLICK HERE TO VIEW ALL AUDIT ENTRIES                               ', command=self.create_window)
    ...
    def handle_label_creation(self):
        ...
        # CHECKBOXES
        self.q1 = tk.Label(self, font=('Calibri', 12),
                           text="External Wrap Intact:")
        self.q2 = tk.Label(self, font=('Calibri', 12),
                           text="Tray Identified:", pady=3)
        self.q3 = tk.Label(self, font=('Calibri', 12),
                           text="Proper Quantity/Sizing:", pady=3)
        self.q4 = tk.Label(self, font=('Calibri', 12),
                           text="Chemical Indicator Included:", pady=3)
        self.q5 = tk.Label(self, font=('Calibri', 12),
                           text="Instruments Visually Clean:", pady=3)
        self.q6 = tk.Label(self, font=(
            'Calibri', 12), text="Instruments Processed Correctly:", pady=3, padx=12)

        # 3 ENTRY FIELDS
        self.e1 = tk.Label(self, font=('Arial', 10),
                           text="Date (dd/mm/yyyy): ", pady=3, padx=10)
        self.e2 = tk.Label(self, font=('Arial', 10),
                           text="Prepared By: ", pady=3, padx=10)
        self.e3 = tk.Label(self, font=('Arial', 10),
                           text="Tray Name: ", pady=3, padx=10)

        # COMMENT LABEL
        self.comment_label = tk.Label(
            self, font=('Arial', 11), text="Comment:")

        # ENTRY COUNT LABEL
        self.counter = tk.Label(self, font=(
            'Calibri', 11), text="Entry Count: " + str(COUNT))

        # LAST ENTRY LABELS
        self.last_entry_header = tk.Label(self, font=(
            'Calibri', 12, 'bold'), text="LAST ENTRY")
        self.last_date = tk.Label(self, font=('Calibri', 8), text='Date:')
        self.last_tray_name = tk.Label(
            self, font=('Calibri', 8), text='Tray ID:')
        self.last_prepared_by = tk.Label(
            self, font=('Calibri', 8), text='Prepared By:')
        self.last_q1 = tk.Label(self, font=(
            'Calibri', 8), text='External Wrap Intact:')
        self.last_q2 = tk.Label(self, font=(
            'Calibri', 8), text='Tray Identified:')
        self.last_q3 = tk.Label(self, font=(
            'Calibri', 8), text='Proper Quantity/Sizing:')
        self.last_q4 = tk.Label(self, font=(
            'Calibri', 8), text='Chemical Indicator Included:')
        self.last_q5 = tk.Label(self, font=(
            'Calibri', 8), text='Instruments Visually Clean:')
        self.last_q6 = tk.Label(self, font=(
            'Calibri', 8), text='Instruments Processed Correctly:')
    ...
    def handle_entry_creation(self):
        ...
        self.prepared_by_combobox = ttk.Combobox(
            self, width=34, values=self.employee_list)
        self.prepared_by_combobox.bind(
            '<FocusOut>', self.focus_prepared_by_validation)
        ...
        self.tray_entry = tk.Entry(
            self, textvariable=self.search_var, width=37)

        self.scrollbar = tk.Scrollbar(self, orient='vertical')
        self.scrollbar.grid(row=10, column=2, sticky='ns')

        self.lbox = tk.Listbox(
            self, width=60, height=10, activestyle='none', yscrollcommand=self.scrollbar.set)
    ...
    def handle_button_creation(self):
        ...
        self.report_button = tk.Button(self, font=(
            'Arial', 10), text=" Create Report ", command=self.create_report)
        self.submit_button = tk.Button(self, font=(
            'Arial', 16, 'bold'), text="SUBMIT", command=self.on_button)

    def handle_layout(self):

        # CHECKBOXES AND THEIR LABELS
        self.q1.grid(row=0, column=0, sticky='', pady=(12, 3))
        self.check_1.grid(row=0, column=1, sticky='', pady=(12, 3))
        self.check_1.state(['!alternate'])
        # Remove 'alternate' state and set default to 'selected'
        self.check_1.state(['selected'])

        self.q2.grid(row=1, column=0, sticky='')
        self.check_2.grid(row=1, column=1, sticky='')
        self.check_2.state(['!alternate'])
        self.check_2.state(['selected'])

        self.q3.grid(row=2, column=0, sticky='')
        self.check_3.grid(row=2, column=1, sticky='')
        self.check_3.state(['!alternate'])
        self.check_3.state(['selected'])

        self.q4.grid(row=3, column=0, sticky='')
        self.check_4.grid(row=3, column=1, sticky='')
        self.check_4.state(['!alternate'])
        self.check_4.state(['selected'])

        self.q5.grid(row=4, column=0, sticky='')
        self.check_5.grid(row=4, column=1, sticky='')
        self.check_5.state(['!alternate'])
        self.check_5.state(['selected'])

        self.q6.grid(row=5, column=0, sticky='')
        self.check_6.grid(row=5, column=1, sticky='')
        self.check_6.state(['!alternate'])
        ...
        # HORIZONTAL LINE
        self.h_line.grid(row=6, column=0, sticky='we',
                         columnspan=2, pady=10, padx=12)

        # 3 ENTRY FIELDS AND THEIR LABELS
        self.e1.grid(row=7, column=0, sticky='w', padx=5)
        self.date.grid(row=7, columnspan=2, padx=(5, 15), sticky='e')
        self.e2.grid(row=8, column=0, sticky='w', padx=5)
        self.prepared_by_combobox.grid(
            row=8, columnspan=2, padx=(5, 15), sticky='e')
        self.e3.grid(row=9, column=0, sticky='w', padx=5)
        self.tray_entry.grid(row=9, columnspan=2,
                             padx=(5, 15), pady=3, sticky='e')

        # LISTBOX
        self.lbox.grid(row=10, columnspan=2, padx=10, pady=10)

        # HORIZONTAL LINE
        self.h_line_2.grid(row=11, column=0, sticky='we',
                           columnspan=2, pady=10, padx=12)

        # COMMENT BOX
        self.comment_label.grid(row=12, columnspan=2)
        self.comment.grid(row=13, columnspan=2, pady=10)

        # CREATE REPORT BUTTON + SUBMIT BUTTON + ENTRY COUNTER
        self.report_button.grid(
            row=14, column=0, sticky='w', padx=15, pady=(0, 7))
        self.submit_button.grid(row=14, pady=(20, 30), columnspan=2)
        self.counter.grid(row=14, column=1, padx=30, pady=(0, 11))

################################################################################################################

    # BIND METHOD ON A WIDGET PASSES THE EVENT OBJECT TO THE CALLBACK FUNCTION
    def focus_date_validation(self, event=None):
        ...
        try:
            ...
            # If so, change the background color to green
            self.date.config({"background": "Green"})
    ...
    def focus_tray_name_validation(self, event=None):
        ...
        if self.lbox.get('active').strip() in self.tray_list and len(self.lbox.curselection()) != 0:
            ...
            self.tray_to_delete = self.lbox.get('active').strip()
...
################################################################################################################
    ...
    def date_validation(self, event=None):
        ...
        try:
            ...
            # If so, change the background color to green
            self.date.config({"background": "Green"})
            # Alter the dictionary value to show that there's no error
            self.errors['date'] = 0
        except:
            self.errors['date'] = 1
            self.date.config({"background": "Red"})
    ...
    def prepared_by_validation(self, event=None):
        ...
        for employee in self.employee_list:
            if self.prepared_by_combobox.get().lower().strip() == employee[:length].lower():
                employees_matched.append(employee)
        ...
        if len(employees_matched) == 1:
            ...
            if self.prepared_by_combobox.get() != '':
                tk.messagebox.showinfo(
                    'Input Error', 'Please enter a valid entry into \'Prepared By:\'')
...
################################################################################################################
    ...
    def get_checkbox_answers(self):
        ...
        try:
            ...
            if (self.check_1.state()[0] == 'selected' or self.check_1.state()[1] == 'selected'):
                self.q1_answer = 'Yes'
        except:
            pass
    ...
    def display_last_entry(self):
        ...
        # HORIZONTAL LINE
        self.h_line_3.grid(row=15, column=0, sticky='we',
                           columnspan=2, pady=(0, 15), padx=12)
        ...
        self.last_date_answer = tk.Label(self, font=(
            'Calibri', 8), text=str(self.df.iloc[self.df.shape[0]-1][0]))
        self.last_date_answer.grid(row=17, column=1)

        self.last_tray_name_answer = tk.Label(self, font=(
            'Calibri', 8), text='[' + str(self.df.iloc[self.df.shape[0]-1][1].split('[')[1]))
        self.last_tray_name_answer.grid(row=18, column=1)

        self.last_prepared_by_answer = tk.Label(self, font=(
            'Calibri', 8), text=str(self.df.iloc[self.df.shape[0]-1][2]))
        self.last_prepared_by_answer.grid(row=19, column=1)

        self.last_q1_answer = tk.Label(self, font=(
            'Calibri', 8), text=str(self.df.iloc[self.df.shape[0]-1][3]))
        self.last_q1_answer.grid(row=20, column=1)

        self.last_q2_answer = tk.Label(self, font=(
            'Calibri', 8), text=str(self.df.iloc[self.df.shape[0]-1][4]))
        self.last_q2_answer.grid(row=21, column=1)

        self.last_q3_answer = tk.Label(self, font=(
            'Calibri', 8), text=str(self.df.iloc[self.df.shape[0]-1][5]))
        self.last_q3_answer.grid(row=22, column=1)

        self.last_q4_answer = tk.Label(self, font=(
            'Calibri', 8), text=str(self.df.iloc[self.df.shape[0]-1][6]))
        self.last_q4_answer.grid(row=23, column=1)

        self.last_q5_answer = tk.Label(self, font=(
            'Calibri', 8), text=str(self.df.iloc[self.df.shape[0]-1][7]))
        self.last_q5_answer.grid(row=24, column=1)

        self.last_q6_answer = tk.Label(self, font=(
            'Calibri', 8), text=str(self.df.iloc[self.df.shape[0]-1][8]))
    ...
    def save_files(self, employee_name):
        ...
        self.timestamp = str(datetime.now().date()) + ' ' + str(datetime.now().hour) + \
            '-' + str(datetime.now().minute) + '-' + str(datetime.now().second)

        workbook.save('C:\\Path\\To\\Audit Data Folder - ' +
                      CURRENT_YEAR + ' - ' + employee_name + '.xlsx')
        workbook.save('C:\\Path\\To\\Audit Data BACKUP Folder - ' +
                      self.timestamp + ' - ' + employee_name + '.xlsx')
    ...
    def create_report(self):
        ...
        for employee_name in self.employee_list:
            ...
            try:
                ...
                self.imported_audit_df = pd.read_excel(
                    'C:\\Path\\To\\Audit Data Folder - ' + CURRENT_YEAR + ' - ' + employee_name + '.xlsx')
                self.audited_list = list(self.imported_audit_df['Tray Name'])
                self.audited_list = [x.strip() for x in self.audited_list]

                self.master_tray_df = pd.read_excel(
                    'C:\\Path\\To\\master_tray_list.xlsx')
                ...
                for row in range(self.master_tray_df.shape[0]):
                    if self.master_tray_df.iloc[row]['Tray Name'] in self.audited_list:
                        ...
                        self.report_df = self.report_df.append(
                            self.master_tray_df.loc[row, ['File', 'Tray Name', 'ID']])
                ...
                for row_index, row in enumerate(rows_report, 1):
                    for column_index, value in enumerate(row, 1):
                        worksheet_report.cell(
                            row=row_index, column=column_index, value=value).alignment = Alignment(horizontal='center')

                workbook_report.save('C:\\Path\\To\\Audit Report - ' +
                                     CURRENT_YEAR + ' - ' + employee_name + '.xlsx')
